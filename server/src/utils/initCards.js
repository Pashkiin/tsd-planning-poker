...
// Funkcja do inicjalizacji kart w bazie danych
const initializeCards = async () => {
  try {
    // Sprawdź, czy karty już istnieją w bazie danych
    const existingCards = await Card.find();

    if (existingCards.length === 0) {
      // Jeśli brak kart w bazie, wstaw wszystkie
      await Card.insertMany(cards);
      console.log("Karty zostały zainicjowane w bazie danych.");
    } else {
      console.log("Karty już istnieją w bazie danych.");
    }
  } catch (error) {
    console.error("Błąd podczas inicjalizacji kart: ", error);
  }
};
